...
public interface RepairRepository extends JpaRepository<RepairEntity, Long> {
    ...
    @Query("SELECT rep.reparationType, SUM(rep.totalAmount) AS sumita, COUNT(rep.reparationType), veh.type " +
            "FROM repair rep " +
            "JOIN rep.vehicle veh " +
    ...
    // TODO: Verificar que funcionen con "rep.vehicle veh"
    @Query("SELECT rep.reparationType, SUM(rep.totalAmount) AS sumita, COUNT(rep.reparationType), veh.motor " +
            "FROM repair rep " +
            "JOIN rep.vehicle veh " +
            "GROUP BY rep.reparationType, veh.motor " +
            "ORDER BY sumita DESC")
    List<Object[]> getMotorReport();
}
